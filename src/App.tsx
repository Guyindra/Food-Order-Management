...
export default function App() {
  ...
  useEffect(() => {
    socket.on("initial_state", (state: { menu: MenuItem[], tables: Table[], orders: Order[] }) => {
      setMenuItems(state.menu);
      setTables(state.tables);
      setOrders(state.orders.sort((a, b) => b.createdAt - a.createdAt));
      setLoading(false);
    });
    ...
    socket.on("order_updated", (order) => {
      setOrders(prev => [order, ...prev.filter(o => o.id !== order.id)].sort((a, b) => b.createdAt - a.createdAt));
    });

    socket.on("order_status_changed", ({ id, status, updatedAt }) => {
      setOrders(prev => prev.map(o => o.id === id ? { ...o, status, updatedAt } : o).sort((a, b) => b.createdAt - a.createdAt));
    });
  ...
  const updateOrderStatus = (orderId: string, status: OrderStatus) => {
    socket.emit("update_order_status", { id: orderId, status });
  };
  ...
  return (
    <div className="min-h-screen bg-brand-bg flex overflow-hidden">
        ...
        <nav className="hidden lg:flex w-24 bg-brand-orange flex-col items-center py-8 gap-10 border-r-4 border-black/10 shrink-0">
          ...
          <div className="flex flex-col gap-8">
            <button 
              ...
              className={`w-12 h-12 rounded-xl flex items-center justify-center cursor-pointer transition-all ${role === UserRole.CUSTOMER ? 'bg-white shadow-md text-brand-orange' : 'bg-white/20 text-white hover:bg-white/40'}`}
            >
              <Utensils className="w-6 h-6" />
            </button>
            <button 
              onClick={() => setRole(UserRole.KITCHEN)}
              className={`w-12 h-12 rounded-xl flex items-center justify-center cursor-pointer transition-all ${role === UserRole.KITCHEN ? 'bg-white shadow-md text-brand-orange' : 'bg-white/20 text-white hover:bg-white/40'}`}
            >
              <ChefHat className="w-6 h-6" />
            </button>
            <button 
              onClick={() => setRole(UserRole.CASHIER)}
              className={`w-12 h-12 rounded-xl flex items-center justify-center cursor-pointer transition-all ${role === UserRole.CASHIER ? 'bg-white shadow-md text-brand-orange' : 'bg-white/20 text-white hover:bg-white/40'}`}
            >
              <CreditCard className="w-6 h-6" />
            </button>
            <button 
              onClick={() => setRole(UserRole.ADMIN)}
              className={`w-12 h-12 rounded-xl flex items-center justify-center cursor-pointer transition-all ${role === UserRole.ADMIN ? 'bg-white shadow-md text-brand-orange' : 'bg-white/20 text-white hover:bg-white/40'}`}
            >
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="h-24 px-6 md:px-10 flex items-center justify-between border-b border-brand-border bg-white lg:bg-transparent">
          <div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    {tables.map(t => (
                      <button
                        ...
                        className="bg-white border-2 border-brand-border p-6 rounded-[32px] hover:border-brand-orange hover:translate-y-[-4px] shadow-sm transition-all flex flex-col items-center gap-2 group"
                      >
              ...
              <>
                <div className={`flex-1 overflow-y-auto p-4 md:p-8 transition-all ${view !== 'menu' ? 'hidden md:block' : ''}`}>
                  <div className="flex items-center justify-between mb-8">
                    <div>
                      <h2 className="text-3xl font-black tracking-tight">Table {selectedTable.tableNumber}</h2>
                  ...
                  <div className="flex gap-4 mb-8 overflow-x-auto pb-4 scrollbar-none">
                    {['All', ...Array.from(new Set(menuItems.map(m => m.category)))].map(cat => (
                      <button key={cat as string} className="px-6 py-2.5 bg-white border-2 border-brand-border rounded-full text-sm font-black whitespace-nowrap hover:border-brand-orange hover:text-brand-orange transition-all">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-8">
                    {menuItems.map(item => (
                      <motion.div 
                        ...
                        key={item.id} 
                        className="bg-white rounded-[32px] overflow-hidden border-2 border-brand-border hover:border-brand-teal group flex flex-col transition-all"
                      >
                        <div className="h-56 bg-brand-border relative overflow-hidden">
                          <img 
                          ...
                          <div className="absolute top-4 right-4">
                            <span className="bg-white px-4 py-1 rounded-full font-black text-brand-teal shadow-md border-2 border-brand-teal/10">
                              ${item.price.toFixed(2)}
                            </span>
                          </div>
                        </div>
                        <div className="p-6 flex-1 flex flex-col">
                          <h3 className="font-black text-xl mb-2 tracking-tight">{item.name}</h3>
                          <p className="text-brand-muted text-sm font-medium line-clamp-2 mb-6 flex-1">{item.description}</p>
                          <button
                            onClick={() => addToCart(item)}
                            disabled={!item.isAvailable}
                            className="w-full bg-brand-orange text-white py-4 rounded-2xl font-black flex items-center justify-center gap-2 hover:translate-y-[-2px] shadow-[0_4px_0_0_#D85A2B] active:translate-y-[2px] active:shadow-none transition-all disabled:opacity-50"
                          >
                ...
                <AnimatePresence>
                  {(view === 'cart' || view === 'status') && (
                    <motion.div 
                      ...
                      animate={{ x: 0 }}
                      exit={{ x: '100%' }}
                      className="fixed inset-0 z-50 md:relative md:inset-auto md:w-96 bg-white flex flex-col shadow-2xl md:shadow-none border-l-4 border-black/5"
                    >
                      <div className="p-6 border-b border-brand-border flex items-center justify-between">
                        <button onClick={() => setView('menu')} className="md:hidden">
                          <MoveRight className="w-6 h-6 rotate-180" />
                        </button>
                        <h3 className="font-black text-2xl tracking-tight">Table {selectedTable.tableNumber}</h3>
                        ...
                        {view === 'cart' ? (
                                ...
                                {cart.map(c => (
                                  <div key={c.item.id} className="flex flex-col gap-3 group border-b border-brand-border pb-4 last:border-0">
                                    ...
                                    <input 
                                      ...
                                      className="text-xs font-medium bg-brand-bg/50 px-3 py-2 rounded-lg border border-brand-border outline-none focus:border-brand-orange focus:bg-white w-full text-brand-text placeholder-brand-muted/70 transition-colors"
                                    />
                                  </div>
                                ))}
                              </div>
                          ...
                          <div className="space-y-8">
                            {orders.filter(o => o.tableId === selectedTable.id && o.status !== OrderStatus.PAID).map(order => (
                              <div key={order.id} className="bg-white rounded-[32px] border-2 border-brand-orange/20 overflow-hidden shadow-sm">
                      ...
                      {view === 'cart' && cart.length > 0 && (
                        <div className="p-6 border-t-4 border-brand-bg bg-white shadow-inner">
                          <div className="flex justify-between items-end mb-6">
                            <span className="text-sm font-black text-brand-muted uppercase tracking-widest">Bag Total</span>
                            <span className="text-3xl font-black text-brand-orange">${cart.reduce((a, b) => a + (b.item.price * b.quantity), 0).toFixed(2)}</span>
                          </div>
                          <button
                            onClick={placeOrder}
      ...
      {!isPublicCustomer && (
        <div className="fixed bottom-4 left-4 z-40">
           <div className="group relative">
              ...
              <div className="absolute bottom-full left-0 mb-2 w-64 bg-white p-4 rounded-xl shadow-xl border hidden group-hover:block transition-all animate-in fade-in slide-in-from-bottom-2">
